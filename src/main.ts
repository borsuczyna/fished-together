...
import './style.css';
import { Vector3D, Size } from './uitls/position';

engine.render.lightColor = new Color(425, 425, 425);
engine.render.lightDirection.set(2, -2).normalize();
engine.render.normalPower = .7;

engine.camera = new FreeCam(engine);
let blocks: Vector3D[] = [];
for(let x = 0; x < 31; x++) {
    blocks.push(new Vector3D(x, 0, 0));
}
...
function update() {
    ...
    engine.render.drawImage3DWithNormal(new Vector3D(-512, 512, 0), new Size(1024, 1024), '/gun.png', '/gun-normal.png');
